...
CH32V305 microcontroller. And because the CH32V305 can do so much
more than its smaller sibling, the idea of this development board
was born. 

This description is valid for the `lite' variant of said development
board and its second production release version `1.0'.

\includegraphics[scale=0.4]{docmentation/quince_front_kl}
...
version omits the switchable target supply voltages
...
Remark: ports PB8 and PB9 are exchanged in the v0.9 revision of the
\emph{Quince micro lite}.
...
(position U50) with its decoupling capacitor (C51, 100nF) and a four
...
is close to the \emph{WCH LinkE R0 1v3}; the power supply pins are
not switchable. Also not available is the ``ModeSelect'' button.
Its functionality can be controlled via software, as stated in the
``WCH-LinkUserManual'' (version V2.4).
...
\begin{enumerate}
\item Install the software from WCH, commands for Linux:
\begin{enumerate}
...
\leavevmode\egroup
\end{enumerate}
\item Build the software (see respective documentation)
...
\end{enumerate}
Now the Quince micro is ready to function as a WCH-LinkE adapter.
After connecting it to another CH32 target (for example another Quince
micro), the second microcontroller can be programmed with, for example:
\begin{enumerate}
\item \bgroup\inputencoding{latin9}
\begin{lstlisting}
...
\subsection*{Revision v0.9}

First production data.

\subsection*{Revision v1.0}
\begin{itemize}
\item Changed the connection of PB8 and PB9 to match the RaspberryPi pinout,
enabling an additional I\texttwosuperior C bus.
\item Connected 5V and 3.3V to the debug connector (non-switchable supply)
\end{itemize}
