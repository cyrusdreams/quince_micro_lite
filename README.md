...
-   the *LinkE* debug connector is found on the PCB edge, the "lite" version omits the switchable target suply voltages
...
Remark: ports PB8 and PB9 will be exchanged in a later revision of the *Quince micro* to enable an additional IC interface, compatible to the *Raspberry Pi* pinout.
...
On the bottom side of the PCB, a CAN bus transceiver can be mounted (position U50) with its decoupling caacitor (C51, 100nF) and a four pole SMD pin header. With the available signals CAN-high, CAN-low, 5V-supply and Ground, the Quince micro can be supplied with power and act as a CAN bus node.
...
On the short side of the PCB, opposite of the USB-S connector, there are solder pads suitable for two sets of five pin headers. The pinout is close to the *WCH LinkE R0 1v3*; the switchable supply pins are missing. Also not available is the "ModeSelect" button. Its functionality can be controlled via software, as stated in the "WCH-LinkUserManual" (version V2.4).
...
1.  Install the software from WCH installieren, commands for Linux:

    1.  git clone https://github.com/ch32-rs/wchisp

    2.  git clone https://github.com/ch32-rs/wlink

2.  Build the software (see respective dokumentation)
...
Now the Quince micro is ready to function as a WCH-LinkE apter. After connecting it to another CH32 target (for example another Quince micro), the second microcontroller can be programmed with, for example:
